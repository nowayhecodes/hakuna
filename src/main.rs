...
fn main() {
    ...
    let mut address: String = "localhost:7770".to_string();
    ...
    while let Some(arg) = args.next() {
        match arg.as_ref() {
            ...
            "--no-reload" | "--no-refresh" => reload = false,
            "--path" => {
                path = args
                    .next()
            ...
            "--help" => {
                println!(
                    r#"
    Run 'hakuna' in a folder to host that folder. 
    
    --reload                    Automatically refresh pages when a file in the hosted folder changes. Defaults to true.
    --no-reload                 Does the opposite of --reload.
    --address  [address]:[port] Specify an address to use. Defaults to localhost:7770.
    --path     [path]           Specify the path of the folder to be hosted.
    --header                    Specify an additional header to send in responses. Use multiple --header flash for multiple headers.
    --help                      Display this help screen.
    
    Examples: 
    
    hakuna --address 127.0.0.1:7788 --path "path/sub_path" --header Access-Control-Allow-Origin='*' 
                    "#
                );
                return;
            }
            _ => {
                println!(
                    "Unrecognized flag: `{:?}`. See available options with `hakuna --help`",
                    arg
                );
                return;
            }
    ...
    if !std::path::Path::new(&hosted_path).exists() {
        println!("Path [{}] doesn't exists.", hosted_path.display());
        return;
    }
